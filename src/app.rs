...
/// Builds the production router: the canonical route table and layer order.
///
/// `analytics_enabled` toggles the analytics recording layer only; every other
/// layer is applied in both modes so benchmark and test builds match production.
pub fn build_app(state: AppState, analytics_enabled: bool) -> Router {
    ...
    router
        ...
        .layer(MalTokenRefreshLayer::new(state.config.clone()))
