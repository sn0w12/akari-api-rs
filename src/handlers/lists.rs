...
pub async fn get_list(
    ...
    Ok(Json(SuccessResponse {
        ...
        data: UserListDetailResponse {
            ...
            total_entries: list.total_entries as i32,
            entries,
            user: UserResponse {
                user_id: uid,
...
pub async fn add_entry(
    ...
    let owned: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM public.user_lists WHERE id = $1 AND user_id = $2)",
    )
    .bind(list_id)
    .bind(&user.id)
    .fetch_one(&db)
    ...
    let max_order: Option<i32> = sqlx::query_scalar(
        "SELECT MAX(order_index) FROM public.user_list_entries WHERE list_id = $1",
    )
    .bind(list_id)
    .fetch_one(&db)
    .await?;
...
pub async fn remove_entry(
    user: AuthUser,
    Path((list_id, work_id)): Path<(Uuid, Uuid)>,
    State(db): State<DbPool>,
) -> Result<Json<SuccessResponse<serde_json::Value>>, ApiError> {
    let owned: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM public.user_lists WHERE id = $1 AND user_id = $2)",
